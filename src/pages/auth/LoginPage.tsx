...
import { Lock, Mail, ScanEye, ShieldCheck, Stethoscope, UserRound } from "lucide-react";
...
export default function LoginPage() {
  ...
  const { loginAsDemoUser, loginAsPatient, loginWithCredentials } = useAuth();
  const { patients } = usePatients();
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [loading, setLoading] = useState<"creds" | UserRole | null>(null);
  const [patientPickerOpen, setPatientPickerOpen] = useState(false);
  const [clinicianPickerRole, setClinicianPickerRole] = useState<"hausarzt" | "dermatologist" | null>(null);
  ...
  function demoLogin(role: UserRole) {
    ...
    setClinicianPickerRole(role);
  }

  async function chooseClinician(userId: string) {
    setLoading(clinicianPickerRole);
    const user = await loginAsDemoUser(userId);
    setClinicianPickerRole(null);
    navigate(homeForRole(user.role));
    setLoading(null);
  }
  ...
  async function choosePatient(patientId: string) {
    setLoading("patient");
    const user = await loginAsPatient(patientId);
  ...
  return (
    <>
        ...
        <div className="hidden lg:flex flex-col justify-between p-10 bg-gradient-to-br from-brand-700 via-brand-600 to-brand-800 text-white relative overflow-hidden">
          ...
          <div className="relative">
            <h1 className="text-4xl font-semibold tracking-tight leading-tight">
              Digitale dermatologische Konsile fuer die hausaerztliche Versorgung.
            </h1>
            <p className="mt-4 text-brand-100/90 max-w-md">
              Sichere Befund-Anfragen, strukturierte Bilddokumentation und schnelle
              Expert:innen-Einschaetzungen - in einer Plattform.
            </p>
            <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-md">
              ...
              <Feature icon={<Mail className="w-4 h-4" />} title="Sichere Kommunikation" desc="Direkter Rueckfragen-Thread." />
            </div>
          </div>
          <div className="relative text-xs text-brand-100/80">
            (c) {new Date().getFullYear()} Derma Consult · Demo-Umgebung
          </div>
        </div>
          ...
          <div className="w-full max-w-md">
            ...
            <div className="mb-8">
              ...
              <p className="text-sm text-ink-600 mt-1">
                Willkommen zurueck. Bitte melden Sie sich an, um fortzufahren.
              </p>
            </div>
      ...
      <Modal
        ...
        title="Demo-Patient:in auswaehlen"
        description="Fuer die Demo waehlen Sie direkt die Patientenakte aus."
      >
        <div className="space-y-2 max-h-[420px] overflow-auto">
          {patients.map((patient) => (
            <button
              ...
              className="w-full text-left rounded-lg border border-ink-200 bg-white px-3 py-2 hover:bg-ink-50 transition-colors"
            >
              <div className="text-sm font-medium text-ink-900">
                {patient.firstName} {patient.lastName}
              </div>
              <div className="text-xs text-ink-500">geb. {patient.dateOfBirth}</div>
            </button>
          ))}
        </div>
      </Modal>
      ...
      <Modal
        open={!!clinicianPickerRole}
        onClose={() => setClinicianPickerRole(null)}
        title={
          clinicianPickerRole === "hausarzt"
            ? "Demo-Hausarzt:in auswaehlen"
            : "Demo-Dermatolog:in auswaehlen"
        }
        description="Fuer die Demo waehlen Sie direkt einen Account aus."
      >
        <div className="space-y-2 max-h-[420px] overflow-auto">
          {mockUsers
            .filter((user) => user.role === clinicianPickerRole)
            .map((user) => (
              <button
                key={user.id}
                type="button"
                onClick={() => chooseClinician(user.id)}
                className="w-full text-left rounded-lg border border-ink-200 bg-white px-3 py-2 hover:bg-ink-50 transition-colors"
              >
                <div className="text-sm font-medium text-ink-900">{user.name}</div>
                <div className="text-xs text-ink-500">
                  {user.role === "hausarzt" ? user.practiceName : user.department}
                </div>
              </button>
            ))}
        </div>
