              ...
              <CardBody className="flex flex-col sm:flex-row sm:items-center gap-3 sm:justify-between">
                ...
                <div className="flex flex-col sm:flex-row gap-2 shrink-0">
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => setQrLink(lanUploadUrlForToken(konsil.uploadToken || uploadTokenForKonsilId(konsil.id), appOrigin))}
                  >
                    <QrCode className="w-4 h-4" /> QR-Code oeffnen
                  </Button>
                  <Button
                    onClick={() => {
                      window.location.href = lanUploadUrlForToken(konsil.uploadToken || uploadTokenForKonsilId(konsil.id), appOrigin);
                    }}
                  >
